...
- `func_instance` handling is preservation-only with validation warnings; Source Weaver preserves instance entities and transforms their `origin`, but does not resolve search roots, inline referenced VMFs, apply replacement parameters, evaluate fixup styles, expand nested instances, or prove compiler instance-collapse behavior. See `docs/func-instance-handling.md`.
