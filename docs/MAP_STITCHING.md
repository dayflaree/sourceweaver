...
This slice never mutates source inputs. Generated output is a new VMF byte stream and source bytes remain preserved as an exact prefix. Alignment, seam overlap, deletion-class, seam-confidence, ID-allocation, namespace-plan, singleton-conflict, preflight, manifest, materialization, and lifecycle-controller records do not authorize overwriting originals. The implementation does not materialize brush geometry transforms, remove duplicate transition geometry from source files, emit lifecycle controller entities, or run compiler/runtime acceptance gates. FGD-backed namespacing is intentionally limited to the documented built-in conservative keyvalue schema until exact game FGDs are wired in.
